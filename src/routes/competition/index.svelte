...
<script>
  ...
  const activateCompetition = async (competition, i) => {
    try {
      const axiosResp = await (
        await axios.put("api/competition?id=" + competition.id)
      ).data;
      if (axiosResp) {
        handleNotification(
          window,
          `${competition.competitionName} has been made active`,
          EnotificationType.SUCCESS
        );
            ...
            {#each competitions as competition, i}
              <tr>
                ...
                <td
                  ><img
                    class="img"
                    src={competition.image || "images/kata.jpg"}
                    alt=""
                  /></td
                >
                <td
